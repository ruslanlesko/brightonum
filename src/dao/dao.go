...
type UserDao interface {
	Save(*s.User) int
	GetByUsername(string) *s.User
	Get(int) *s.User
}

// MemoryUserDao implementation for UserDao
type MemoryUserDao struct {
	Users    map[int]*s.User
	LatestID int
}

// Save saves user
func (d *MemoryUserDao) Save(u *s.User) int {
	newID := d.LatestID + 1
	d.LatestID = newID
	d.Users[newID] = u
	return newID
...
func (d *MemoryUserDao) GetByUsername(username string) *s.User {
...
func (d *MemoryUserDao) Get(id int) *s.User {
	return d.Users[id]
}
...
func (d *MongoUserDao) GetByUsername(username string) *s.User {
	result := []s.User{}
...
// Get returns user by id
func (d *MongoUserDao) Get(id int) *s.User {
	result := []s.User{}
